...
void c16(StringBuffer sb, String msg, {int fg = 7, int bg = NONE, bool fb = false, bool bb = false}) {
  // 16 color palette
  //  h: [0..7], +60 for high intensity (0=black, 1=red, 2=green, 3=yellow, 4=blue, 5=magenta, 6=cyan, 7=white)
  // fg: \e[Cm, C = 30 + h [+ 60]
  // bg: \e[Cm, C = 40 + h [+ 60]
  if (fg > NONE && fg < 8) { sb.write('${CSI}${30+fg+(fb?60:0)}m'); }
  if (bg > NONE && bg < 8) { sb.write('${CSI}${40+bg+(bb?60:0)}m'); }
  sb.write(msg);
  sb.write(RESET);
}
...
void cRGB(StringBuffer sb, String msg, {int fg = 0x999999, int bg = NONE}) {
  ...
  // fg: \e[38;2;R;G;Bm
  // bg: \e[48;2;R;G;Bm
  if (fg > NONE && fg <= 0xffffff) { sb.write('${CSI}38;2;${(fg>>16)&255};${(fg>>8)&255};${fg&255}m'); }
  if (bg > NONE && bg <= 0xffffff) { sb.write('${CSI}48;2;${(bg>>16)&255};${(bg>>8)&255};${bg&255}m'); }
  sb.write(msg);
  sb.write(RESET);
}

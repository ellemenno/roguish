...
void testAnsi() {
  group('ansi', () {
    ...
    test('c16() sets a default foreground of dim white', () {
      ansi.c16(sb, msg);
      expect('${sb}', equals('\x1B[37m${msg}\x1B[0m'));
    });
    test('c16() can skip both foreground and background', () {
      ansi.c16(sb, msg, fg: -1, bg: -1);
      expect('${sb}', equals('${msg}\x1B[0m'));
    });

    test('c16() ignores fg values under range', () {
      ansi.c16(sb, msg, fg: -1);
      expect('${sb}', equals('${msg}\x1B[0m'));
    });
    test('c16() ignores fg values over range', () {
      ansi.c16(sb, msg, fg: 8);
      expect('${sb}', equals('${msg}\x1B[0m'));
    });
    test('c16() adds 30 for dim fg colors', () {
      ansi.c16(sb, msg, fg: 0);
      expect('${sb}', equals('\x1B[30m${msg}\x1B[0m'));
    });
    test('c16() adds 60 for bright fg colors', () {
      ansi.c16(sb, msg, fg: 0, fb: true);
      expect('${sb}', equals('\x1B[90m${msg}\x1B[0m'));
    });

    test('c16() ignores bg values under range', () {
      ansi.c16(sb, msg, fg: -1, bg: -1);
      expect('${sb}', equals('${msg}\x1B[0m'));
    });
    test('c16() ignores bg values over range', () {
      ansi.c16(sb, msg, fg: -1, bg: 8);
      expect('${sb}', equals('${msg}\x1B[0m'));
    });
    test('c16() adds 40 for dim bg colors', () {
      ansi.c16(sb, msg, fg: -1, bg: 0);
      expect('${sb}', equals('\x1B[40m${msg}\x1B[0m'));
    });
    test('c16() adds 90 for bright bg colors', () {
      ansi.c16(sb, msg, fg: -1, bg: 0, bb: true);
      expect('${sb}', equals('\x1B[100m${msg}\x1B[0m'));
    });

    test('c16() can set dim foreground and background', () {
      ansi.c16(sb, msg, fg: 0, bg: 0);
      expect('${sb}', equals('\x1B[30m\x1B[40m${msg}\x1B[0m'));
    });
    test('c16() can set bright foreground and background', () {
      ansi.c16(sb, msg, fg: 0, bg: 0, fb: true, bb: true);
      expect('${sb}', equals('\x1B[90m\x1B[100m${msg}\x1B[0m'));
    });

    test('cRGB() sets a default foreground of #999999 (dim white)', () {
      ansi.cRGB(sb, msg);
      expect('${sb}', equals('\x1B[38;2;153;153;153m${msg}\x1B[0m'));
    });
    test('cRGB() can skip both foreground and background', () {
      ansi.cRGB(sb, msg, fg: -1, bg: -1);
      expect('${sb}', equals('${msg}\x1B[0m'));
    });

    test('cRGB() ignores fg values under range', () {
      ansi.cRGB(sb, msg, fg: -1);
      expect('${sb}', equals('${msg}\x1B[0m'));
    });
    test('cRGB() ignores fg values over range', () {
      ansi.cRGB(sb, msg, fg: 0xffffff + 1);
      expect('${sb}', equals('${msg}\x1B[0m'));
    });
    test('cRGB() ignores bg values under range', () {
      ansi.cRGB(sb, msg, fg: -1, bg: -1);
      expect('${sb}', equals('${msg}\x1B[0m'));
    });
    test('cRGB() ignores bg values over range', () {
      ansi.cRGB(sb, msg, fg: -1, bg: 0xffffff + 1);
      expect('${sb}', equals('${msg}\x1B[0m'));
    });
    test('cRGB() can set arbitrary rgb colors, like #0818A8 (zaffre)', () {
